"""
Fetch intraday 1-minute historical data from GDFL for ALL available SENSEX expiries.
Saves to sensex_all_expiry_gdfl.duckdb:
  - spot_data:    SENSEX index 1-min bars
  - options_data: SENSEX options 1-min bars (all future expiries, dynamic strike range)

Strike range: spot high/low ± 3000 (computed after spot data is downloaded).
...
SPOT_SYMBOL      = "SENSEX"
STRIKE_PADDING   = 3000          # ± points around day high/low
DB_PATH          = "sensex_all_expiry_gdfl.duckdb"
MASTER_CSV       = "gdfl_master_bse.csv"
...
def load_master_from_csv(csv_path: str) -> List[Dict[str, Any]]:
    if not os.path.exists(csv_path):
        print(f"✗ Master file not found: {csv_path} — run download_bse_master.py first")
        return []
    try:
...
def filter_sensex_all_expiry_symbols(
    instruments: List[Dict[str, Any]], ref_date: Optional[date] = None
) -> List[str]:
    """Return ALL SENSEX options across every future expiry (no strike filter yet)."""
    today = ref_date or datetime.now().date()
    symbols: List[str] = []
    expiries_seen: Set[str] = set()

    for inst in instruments:
        if inst.get("Product", "").strip() != "SENSEX":
            continue
        if inst.get("Name", "").strip() != "IO":
            continue
    ...
    print(f"✓ Loaded {len(symbols)} SENSEX options across {len(expiries_seen)} expiries")
...
def parse_option(identifier: str) -> Optional[Dict[str, Any]]:
    """Parse IO_SENSEX_DDMONYYYY_CE/PE_STRIKE → dict, or None if not an option."""
    if not identifier.startswith("IO_SENSEX_"):
        return None
...
async def main() -> None:
    ...
    print("\nLoading all SENSEX options for ALL expiries...")
    all_symbols = filter_sensex_all_expiry_symbols(instruments, ref_date=today)
    if not all_symbols:
    ...
    try:
        # ── 1. Spot ──────────────────────────────────────────────────────────
        print(f"\n{'='*70}\nFetching SENSEX SPOT ({SPOT_EXCHANGE})\n{'='*70}")
        try:
        ...
        # ── 3. Options ───────────────────────────────────────────────────────
        print(f"\n{'='*70}\nFetching SENSEX OPTIONS ({OPTIONS_EXCHANGE}) — {len(options_symbols)} symbols\n{'='*70}")
