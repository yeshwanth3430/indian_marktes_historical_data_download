"""
GDFL daily TICK downloader — SENSEX index futures & options (BSE / BFO).

For a given trading day it pulls full-session (09:15 -> 15:30) TICK data for
every SENSEX futures & options contract listed in gdfl_master_bse.csv and writes
one CSV per contract, replicating the GFDL daily-dump layout:

    <MON_YEAR>/GFDLBFO_TICK_<DDMMYYYY>/GFDLBFO_TICK_<DDMMYYYY>/
        SENSEX-I.BFO.csv           <- near-month future
        SENSEX-II.BFO.csv          <- next-month future
        SENSEX-III.BFO.csv         <- far-month future
        SENSEX09JUL2667100CE.BFO.csv   <- one file per option (TradeSymbol)
        ...
...
Notes
-----
* Uses the GDFL BSE key (exchange BFO). It is a separate single-session key from
  the NSE one, so it can run alongside the NIFTY downloader — but never run two
  BSE downloaders at once.
* In gdfl_master_bse.csv the underlying is in the 'Product' column (SENSEX) and
  the instrument class is in the 'Name' column: 'IF' = index future, 'IO' = index option.
* Contracts that return no ticks (far-dated / non-trading) are skipped, so only
  contracts that actually traded produce a file.
* Prerequisite: run download_bse_master.py once to create gdfl_master_bse.csv.
"""
...
MASTER_CSV = os.path.join(HERE, "gdfl_master_bse.csv")

# Trading day to download. Defaults to today; override with:  python gdfl_daily_download_bse.py 2026-07-06
...
EXCHANGE    = "BFO"
UNDERLYING  = "SENSEX"           # exact underlying (excludes BANKEX)
SUFFIX      = "BFO"              # ticker/file suffix + dump-folder tag
FUT_NAME    = "IF"              # 'Name' value for index futures
OPT_NAME    = "IO"             # 'Name' value for index options
...
RESUME       = True          # skip contracts already processed (file or manifest)
...
def load_instruments():
    """Return (futures, options) job lists from the master CSV.

    The master's header labels are offset from the data — the underlying index
    lives in the 'Product' column and IF/IO in the 'Name' column.
    """
    futures, options = [], []
    ...
    # options keyed / named by TradeSymbol (e.g. SENSEX09JUL2667100CE)
...
def out_dir_for(day) -> str:
    ...
    folder = f"GFDL{SUFFIX}_TICK_{ddmmyyyy}"          # GFDLBFO_TICK_06072026
    return os.path.join(HERE, mon_year, folder, folder)


def write_csv(out_dir, ticker, rows, day):
    """Write one contract's ticks to <ticker>.<SUFFIX>.csv (sorted oldest->newest)."""
    rows.sort(key=lambda r: r["LastTradeTime"])
    date_str = day.strftime("%d/%m/%Y")
    path = os.path.join(out_dir, f"{ticker}.{SUFFIX}.csv")
    tkr = f"{ticker}.{SUFFIX}"
    ...
    try:
        ...
        for n, job in enumerate(jobs, 1):
            ...
            if RESUME and (ticker in done or
                           os.path.exists(os.path.join(out_dir, f"{ticker}.{SUFFIX}.csv"))):
                resumed += 1
                continue
