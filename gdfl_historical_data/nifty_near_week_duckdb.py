...
and store in DuckDB with same structure as nifty_data.duckdb:
- spot_data: NIFTY spot/index data
- options_data: NIFTY options data
...
def _patch_websockets_for_older_python() -> None:
    ...
    Patch the running loop so websockets.connect() works."""
    try:
...
def _find_nearest_nifty_expiry(
    instruments: List[Dict[str, Any]],
    min_strikes: int = 100,
    ref_date: Optional[date] = None,
) -> Optional[str]:
    """Return the nearest NIFTY OPTIDX expiry (on or after ref_date) that has a
    real options chain.

    The master contains thin, long-dated NIFTY series (e.g. far Thursdays /
    year-end contracts with only a handful of strikes) that are not the active
    weekly/monthly expiry. Counting OPTIDX strikes per expiry and requiring at
    least `min_strikes` filters those out so we pick the genuine near expiry.
    """
    today = ref_date or datetime.now().date()

    # Count OPTIDX strikes per expiry so we can skip sparse (non-active) series.
    strike_counts: Dict[str, int] = {}
    for inst in instruments:
        if (
            inst.get("Product", "").strip() == "NIFTY"
            and inst.get("Name", "").strip() == "OPTIDX"
        ):
            expiry_str = inst.get("Expiry", "").strip()
            strike_counts[expiry_str] = strike_counts.get(expiry_str, 0) + 1

    nearest, min_diff = None, float("inf")
    for expiry_str, count in strike_counts.items():
        ...
        if diff >= 0 and diff < min_diff:
            min_diff, nearest = diff, expiry_str
    return nearest


def filter_nifty_current_week_symbols(
    instruments: List[Dict[str, Any]],
    strike_min: int = 0,
    strike_max: int = 99999,
    ref_date: Optional[date] = None,
) -> List[str]:
    nearest_expiry = _find_nearest_nifty_expiry(instruments, ref_date=ref_date)
    if not nearest_expiry:
        print("Could not find NIFTY expiry dates")
        return []
    ...
    for inst in instruments:
        if (
            ...
            if not identifier:
                continue
            try:
                if "_CE_" in identifier:
                    strike = float(identifier.split("_CE_")[1].split("_")[0])
                elif "_PE_" in identifier:
                    strike = float(identifier.split("_PE_")[1].split("_")[0])
                else:
                    continue
                ...
                continue
    print(f"Loaded {len(symbols)} NIFTY options for expiry {nearest_expiry}")
    return symbols
...
class GDFLHistoryClient:
    ...
    def __init__(self, api_key: str, endpoint: str = WS_ENDPOINT, exchange: str = EXCHANGE):
    ...
    async def get_history_for_symbol(
        ...
        while True:
            ...
            # Check for RequestError or other error message types
            if msg_type == "RequestError" or "Error" in msg_type or data.get("Error"):
                error_msg = data.get("Message", "")
                print(f"  ⚠️  History error for {identifier}: {data}")
                # Raise exception for exchange disabled errors so outer handler can catch it
                if "disabled" in error_msg.lower():
                    raise RuntimeError(f"Exchange {self.exchange} is disabled: {error_msg}")
                # For other errors, return empty list (don't raise)
                return []


def parse_symbol(identifier: str, exchange: str = "NFO") -> Dict[str, Any]:
    """
    Parse GDFL identifier to extract components.
    Examples:
    - NIFTY (from NSE_IDX) -> spot/index
    - OPTIDX_NIFTY_03FEB2026_CE_25000 -> option
    - FUTIDX_NIFTY_03FEB2026_XX_0 -> future (skip)
    - NIFTY-I, NIFTY-II, NIFTY-III -> continuous futures (skip, don't save)
    """
    result = {
        "is_spot": False,
        "is_option": False,
    ...
    # Spot/index: From NSE_IDX exchange, identifier is typically "NIFTY"
    if exchange == "NSE_IDX" and identifier == "NIFTY":
        result["is_spot"] = True
        return result

    # Skip continuous futures (NIFTY-I, NIFTY-II, NIFTY-III)
    if identifier in ["NIFTY-I", "NIFTY-II", "NIFTY-III"]:
        return result  # Don't save these

    # Options: OPTIDX_NIFTY_03FEB2026_CE_25000
    if identifier.startswith("OPTIDX_NIFTY_"):
        parts = identifier.split("_")
        if len(parts) >= 5:
            try:
                expiry_str = parts[2]  # e.g., "03FEB2026"
...
async def main() -> None:
    ...
    # Load master instruments
    instruments = load_master_from_csv("gdfl_master_nse.csv")
    if not instruments:
        print("✗ Could not load gdfl_master.csv")
        return

    # Resolve which day to fetch (today by default, or a historical date via mode 2)
    trade_date = _resolve_trade_date()

    # Load all NIFTY options for nearest week expiry relative to trade_date
    # (strike filter applied after spot)
    print("\nLoading all NIFTY options for nearest week expiry...")
    all_options_symbols = filter_nifty_current_week_symbols(
        instruments, strike_min=0, strike_max=99999, ref_date=trade_date
    )
    if not all_options_symbols:
        print("✗ No symbols found in master")
        return

    print(f"✓ Loaded {len(all_options_symbols)} NIFTY options (all strikes, nearest expiry)")

    # NIFTY spot symbol from NSE_IDX exchange
    # The identifier for NIFTY 50 index in NSE_IDX is "NIFTY 50" (with space)
    spot_symbol = "NIFTY 50"  # From NSE_IDX exchange, not NFO
    print(f"\nSpot symbol: {spot_symbol} (from NSE_IDX exchange)")
    ...
    # Connect to DuckDB
    db_path = "nifty_nearest_week.duckdb"
    ...
    try:
        for trade_date, from_epoch, to_epoch in dates_to_fetch:
            ...
            # Fetch NIFTY spot from NSE_IDX exchange
            print(f"\n{'='*70}")
            print(f"Fetching NIFTY SPOT for {date_str} (NSE_IDX)...")
            print(f"{'='*70}")
            try:
                async with GDFLHistoryClient(API_KEY, WS_ENDPOINT, "NSE_IDX") as spot_client:
                    print(f"\nFetching history for {spot_symbol} (NSE_IDX exchange)...")
                    spot_data_rows = await spot_client.get_history_for_symbol(spot_symbol, from_epoch, to_epoch)

                    if spot_data_rows:
                        print(f"  ✓ Received {len(spot_data_rows)} bars for NIFTY spot")
                        for row in spot_data_rows:
                            ...
                            try:
                                ...
                                dt_utc = datetime.utcfromtimestamp(last_trade_time)
                ...
                error_msg = str(e)
                if "disabled" in error_msg.lower():
                    print(f"\n  ⚠️  NSE_IDX exchange is DISABLED for your API key.")
            ...
            # Compute dynamic strike range from today's spot high/low ± 1000
            ...
            if date_spot:
                ...
                strike_min = day_low  - 1000
                strike_max = day_high + 1000
            ...
            # Fetch options from NFO exchange for this date
            print(f"\n{'='*70}")
            print(f"Fetching NIFTY OPTIONS for {date_str} (NFO)...")
            print(f"{'='*70}")
            async with GDFLHistoryClient(API_KEY, WS_ENDPOINT, EXCHANGE) as options_client:
                for identifier in options_symbols:
                    ...
                    parsed = parse_symbol(identifier, exchange="NFO")
        ...
        if options_rows:
            ...
            # Ensure datetime and dates are proper format
